...
function saveBudget() {
  ...
  if(id) {
    ...
    if(getBudgets().find(b => b.category === cat)) { alert('Budget already exists for this category'); return; }
...
export function renderBudgets(container) {
  ...
  container.innerHTML = `
    <div class="fade-in">
      <div class="header">
        ...
        <button class="btn btn-primary" onclick="window.__openAddBudget()" aria-label="Set budget">
        ...
        ${budgets.length ? budgets.map(b => {
          ...
          return `
            <div class="budget-row">
              ...
              <button class="btn btn-ghost btn-sm btn-icon" onclick="window.__openEditBudget('${b.id}')" title="Edit budget" aria-label="Edit budget for ${cat.name}">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
              </button>
              <button class="btn btn-ghost btn-sm btn-icon" onclick="window.__deleteBudget('${b.id}')" title="Delete budget" aria-label="Delete budget for ${cat.name}">
  ...
  window.__openAddBudget = openAddBudget;
  window.__openEditBudget = openEditBudget;
  window.__deleteBudget = deleteBudgetHandler;
  window.__saveBudget = saveBudget;
}
